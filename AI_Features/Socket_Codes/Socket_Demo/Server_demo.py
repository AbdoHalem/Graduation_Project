...
def Sending(client_socket, stop_flag):
    while not stop_flag["stop"]:
        try:
            msg = input()
            if msg.lower() == 'exit':
                client_socket.send(bytes("Server has closed the connection.", "utf-8"))
                # client_socket.close()
                stop_flag["stop"] = True
                break
...
try:
    while True:
        ...
        send_thread = Thread(target=Sending, args=(client_socket, stop_flag))
        recv_thread = Thread(target=Receiving, args=(client_socket, stop_flag))
        send_thread.start()
        recv_thread.start()
        send_thread.join()
        recv_thread.join()
